...
obj_repr = SimpleNamespace(
    ...
    pattern_from_str=lambda name: globals()[name] if name in globals() else __builtins__[name],
...
class SignoffPatternTests(SimpleTestCase):
    ...
    def test_optional(self):
        ...
        self.assertEqual(s.terms(), {str, })
        match = s.match()
        self.assertMatch(match, True, False, [str])  # No pattern is complete unless it has at least one token!
        match = s.match('ABC')
        self.assertMatch(match, True, True, [])
        match = s.match(123, 'ABC')
        self.assertMatch(match, False, False, [])
    ...
    def test_zero_or_more(self):
        ...
        self.assertEqual(s.terms(), {str, })
        match = s.match()
        self.assertMatch(match, True, False, [str])  # no pattern is compelte without at least one token
        match = s.match('ABC')
    ...
    def test_series_with_optionals(self):
        ...
        match = s.match('ABC')
        self.assertMatch(match, True, True, [str, int])  # pattern complete, but optionals are possible
    ...
    def test_pattern_set(self):
        ...
        self.assertEqual(tuple(type(t) for t in s), (AtLeastN, ExactlyOne, AtLeastN, ExactlyOne))
    ...
    def test_in_parallel(self):
        ...
        self.assertMatch(match, False, False, [])
        match = s.match('ABC', 'DEF', object(), object(),)
...
class SignoffPatternExampleUsageTests(SimpleTestCase):
    ...
    def test_objective_function(self):
        ...
        self.assertMatch(match, True, True, [A, ])
        match = self.pattern.match(B(), B(), A(), C(), A(), A(),)
        self.assertMatch(match, True, False, [A, ])
        match = self.pattern.match(B(), A(), )
        self.assertMatch(match, True, False, [C, ])
        match = self.pattern.match(A(), B(), )
