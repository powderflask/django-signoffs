...
class RegexMatchTests(SimpleTestCase):
    ...
    def test_wrap(self):
        ...
        self.assertEqual(wrap('A.B-C**'), Pattern(regex='((?P<ABC>A\\.B\\-C\\*\\*) )', tokens=['A.B-C**']))

    def test_exacly_one(self):
        self.assertEqual(exactly_one('A.B'), Pattern(regex='((?P<AB>A\\.B) )', tokens=['A.B']))

    def test_zero_or_one(self):
        self.assertEqual(zero_or_one('A.B'), Pattern(regex='(((?P<AB>A\\.B) ){0,1})', tokens=['A.B']))

    def test_zero_or_more(self):
        self.assertEqual(zero_or_more('A.B'), Pattern(regex='(((?P<AB>A\\.B) )*)', tokens=['A.B']))

    def test_exactly_n(self):
        self.assertEqual(exactly_n('A.B', 2), Pattern(regex='((?P<AB>A\\.B) ){2,2}', tokens=['A.B']))

    def test_one_or_more(self):
        self.assertEqual(one_or_more('A.B'), Pattern(regex='(((?P<AB>A\\.B) )+)', tokens=['A.B']))
        self.assertEqual(one_or_more(Pattern(regex='xx', tokens=['xx'])), Pattern(regex='(xx+)', tokens=['xx']))

    def test_n_or_more(self):
        self.assertEqual(n_or_more('A.B', 2), Pattern(regex='(((?P<AB>A\\.B) ){2,})', tokens=['A.B']))

    def test_in_series(self):
        self.assertEqual(in_series('A', 'B'), Pattern(regex='(((?P<A>A) )((?P<B>B) ))', tokens=['A', 'B']))
        self.assertEqual(in_series(exactly_one('A'), one_or_more('B')),
                         Pattern(regex='(((?P<A>A) )(((?P<B>B) )+))', tokens=['A', 'B']))

    def test_all_of(self):
        self.assertEqual(all_of('A', 'B'), Pattern(regex='(((?P<A>A) )((?P<B>B) )|((?P<B>B) )((?P<A>A) ))',
                                                   tokens=['A', 'B']))
        self.assertEqual(all_of('A', in_series('B', one_or_more('C'))),
                         Pattern(
    ...
    def test_one_of(self):
        self.assertEqual(one_of('A', 'B', 'C'),
                         Pattern(
                             regex='(((?P<A>A) )|((?P<B>B) )|((?P<C>C) ))',
                             tokens=['A', 'B', 'C'])
                         )
...
class RegexPatternMatcherTests(SimpleTestCase):
    ...
    def test_5(self):
        matcher = PatternMatcher(in_series(one_or_more('A.B'), exactly_one('B-B'), n_or_more('C*C', 2)))
        m = matcher.match('A.B')
    ...
    def test_6(self):
        matcher = PatternMatcher(all_of(one_or_more('A'), exactly_one('B'), n_or_more('C', 2)))
        m = matcher.match('A')
    ...
    def test_7(self):
        matcher = PatternMatcher(all_of(one_or_more('A'), in_series(exactly_one('B'), n_or_more('C', 2))))
        m = matcher.match('A')
    ...
    def test_8(self):
        matcher = PatternMatcher(in_series(one_or_more('A'), all_of(exactly_one('B'), n_or_more('C', 2))))
        m = matcher.match('A')
        self.assertMatch(m, True, False, ['A', 'B', 'C'])
    ...
    def test_9(self):
        matcher = PatternMatcher(all_of(
            exactly_one('A'), n_or_more('B', 2), one_or_more('C')
        ))
        m = matcher.match('C B B B A')
        self.assertMatch(m, True, True, [])

    def test_10(self):
        matcher = PatternMatcher(one_of(
            exactly_one('A'), n_or_more('B', 2), one_or_more('C')
        ))
        m = matcher.match('A')
        self.assertMatch(m, True, True, [])
    ...
    def test_11(self):
        ...
        m = matcher.match('')
        self.assertMatch(m, True, False, ['A'])  # No pattern is complete without at least one token!
        m = matcher.match('A')
        self.assertMatch(m, True, True, [])
        m = matcher.match('A A')
        self.assertMatch(m, False, False, [])
