...
def regex_pattern(pattern, to_str):
    ...
    pattern = [
        p.regex_pattern() if isinstance(p, SigningOrderPattern) else to_str(p) for p in pattern
    ]
    return pattern
...
class PatternSet(SigningOrderPattern):
    ...
    def __init__(self, *pattern, **kwargs):
        ...
        pattern = tuple(token if isinstance(token, SigningOrderPattern) else ExactlyOne(token) for token in pattern)
