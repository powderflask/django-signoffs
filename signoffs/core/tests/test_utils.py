...
class UtilsTests(SimpleTestCase):
    def test_regexs(self):
        self.assertEqual(utils.split_caps_run.sub(r'\1_\2', 'abstractHTTPResponseCode'), 'abstractHTTP_ResponseCode')

    def test_camel_to_snake(self):
        self.assertEqual(utils.camel_to_snake('camel2_camel2_case'), 'camel2_camel2_case')
        self.assertEqual(utils.camel_to_snake('NormalClassName'), 'normal_class_name')
        self.assertEqual(utils.camel_to_snake('abstractHTTPResponseCode'), 'abstract_http_response_code')
        self.assertEqual(utils.camel_to_snake('HTTPResponseCodeXYZ'), 'http_response_code_xyz')
