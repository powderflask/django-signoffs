...
signoff_type = ActionsTestsSignoff.register(
    id="test.actions_signoff",
    logic=SignoffLogic(perm="auth.add_signoff")
)
...
class ActionsSignoffFormTests(TestCase):
    ...
    def test_invalid_signoff(self):
        bf = self.get_signoff_form(data=dict(signed_off="True", signoff_id="invalid.type"))
...
class BasicUserSignoffActionsTests(TestCase):
    ...
    def test_create(self):
        ...
        self.assertEqual(action.get_signoff_type(), signoff_type)

    def test_is_valid_signoff_request(self):
        u = fixtures.get_user()
        u_action = actions.BasicUserSignoffActions(u, self.data)
        self.assertFalse(u_action.is_valid_signoff_request(signoff_type()))

        action = actions.BasicUserSignoffActions(self.user, self.data)
        self.assertTrue(action.is_valid_signoff_request(signoff_type()))
    ...
    def test_is_valid_revoke_request(self):
        ...
        self.assertFalse(u_action.is_valid_revoke_request(action.signoff))

        r_action = actions.BasicUserSignoffActions(self.user, self.revoke_data())
        self.assertTrue(r_action.is_valid_revoke_request(action.signoff))
...
class ActionsApproval(BaseApproval):
    ...
    first_signoff = models.ApprovalSignoff.register(
        id="test.approval.actions.first",
        logic=SignoffLogic(perm="auth.add_signoff")
    )
    final_signoff = models.ApprovalSignoff.register(
        id="test.approval.actions.final",
        logic=SignoffLogic(perm="auth.add_signoff")
    )

    signing_order = so.SigningOrder(
        first_signoff, final_signoff
    )
...
class BasicUserApprovalActionsTests(TestCase):
    ...
    def test_create(self):
        action = actions.BasicUserApprovalActions(self.user, self.get_data(), self.approval)
        self.assertEqual(type(action.signoff_actions), actions.BasicUserSignoffActions)
        self.assertEqual(action.signoff_actions.get_signoff_type(), self.approval.first_signoff)

    def test_is_valid_approval_signoff_request(self):
        u = fixtures.get_user()
        u_action = actions.BasicUserApprovalActions(u, self.get_data(), self.approval)
        self.assertFalse(u_action.is_valid_approval_signoff_request(self.approval.first_signoff()))

        action = actions.BasicUserApprovalActions(self.user, self.get_data(), self.approval)
        self.assertTrue(action.is_valid_approval_signoff_request(self.approval.first_signoff()))
    ...
    def test_is_valid_revoke_signoff_request(self):
        ...
        u = fixtures.get_user()
        u_action = actions.BasicUserApprovalActions(u, self.revoke_data(signoff=signoff), self.approval)
        self.assertFalse(u_action.is_valid_revoke_signoff_request(signoff))

        r_action = actions.BasicUserApprovalActions(self.user, self.revoke_data(signoff=signoff), self.approval)
        self.assertTrue(r_action.is_valid_revoke_signoff_request(signoff))

    def test_revoke_signoff(self):
        r_action = actions.BasicUserApprovalActions(self.user, self.revoke_data(), self.approval)
...
class FsmActionsProcessModel(Model):
    ...
    approval2, approval2_stamp = ApprovalField(ActionsApproval.register('signoffs.test.actions_approval2'))

    process = signoffs_process.FsmApprovalsProcess()

    # FSM approval state transitions
    @process.approval_transition(approval1, state, source=States.STATE0, target=States.STATE1)
    def transition1(self, approval):
        pass

    @process.revoke_transition(approval1, state, source=States.STATE1, target=States.STATE0)
...
class ApprovalProcessUserActionsTests(TestCase):
    ...
    def test_create(self):
        action = actions.ApprovalProcessUserActions(self.user, self.get_data(), self.approval_process)
        self.assertEqual(type(action.signoff_actions), actions.BasicUserSignoffActions)
        self.assertEqual(action.signoff_actions.get_signoff_type(),
    ...
    def test_is_valid_approval_revoke_request(self):
        action = actions.ApprovalProcessUserActions(self.user, self.get_data(), self.approval_process)
        action.sign_signoff()
        r_action = actions.ApprovalProcessUserActions(self.user, {}, self.approval_process)
    ...
    def test_revoke_signoff(self):
        action = actions.ApprovalProcessUserActions(self.user, self.get_data(), self.approval_process)
        action.sign_signoff()
        r_action = actions.ApprovalProcessUserActions(self.user, self.revoke_data(action.signoff),
    ...
    def test_approve_request(self):
        ...
        action = actions.ApprovalProcessUserActions(self.user, {}, self.approval_process)
        action.approve()
