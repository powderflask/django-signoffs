...
class SignoffPermissionTests(TestCase):
    ...
    def test_cant_revoke_without_permission(self):
        ...
        perm = fixtures.get_perm('can_sign')
        u2 = fixtures.get_user(perms=('no_signie', perm,))
...
class SignoffRevokeTests(TestCase):
    ...
    def test_revoke_with_model(self):
        ...
        revoked = models.RevokeReportSignet.objects.select_related('signet').get(signet=signoff.signet)
...
class SignoffRelationsTests(TestCase):
    ...
    def test_signoff_set(self):
        ...
        self.assertSetEqual({so.signet for so in self.r.signoffs.all()}, set(self.signets))
    ...
    def test_signet_set_revoke(self):
        ...
        self.assertEqual(models.ReportSignet._base_manager.filter(report=self.r).count(), len(self.signets))
